...
const colorConfig: Record<string, { text: string; border: string; bg: string; glow: string }> = {
	windows: {
		text: "text-blue-400",
...
export function CategoryChip({ category, version, isCompound }: CategoryChipProps) {
	...
	return (
		<div className="flex flex-wrap items-center gap-2">
			<a
				...
				className={`
					group inline-flex items-center gap-2 px-4 py-2 text-xs font-medium
			...
			{isCompound && (
				<span className="inline-flex items-center gap-2 px-4 py-2 text-xs font-medium rounded-full border backdrop-blur-sm text-cyan-400 border-cyan-500/50 bg-cyan-500/10">
					<HiOutlineRectangleStack className="h-3.5 w-3.5" />
					<span>Compound</span>
				</span>
			)}

			<span className="inline-flex items-center px-3 py-2 text-xs font-medium rounded-full border backdrop-blur-sm text-zinc-400 border-white/[0.08] bg-white/[0.04]">
				v{version}
			</span>
		</div>
	);
