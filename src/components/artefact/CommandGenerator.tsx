...
`;
		...
		for (const entry of targetPaths) {
			...
			const sourcePath = entry.path.replace("C:", source);
...
`;
		...
		for (const entry of targetPaths) {
			...
			if (entry.fileMask) {
				...
				lines.push(`xcopy "${sourcePath}*" "${destPath}\\" /E /H /Y /Q 2>nul`);
			}
			lines.push("");
		}
...
`;
		...
		for (const entry of targetPaths) {
			const safeName = entry.name.replace(/[^a-zA-Z0-9]/g, "_");
			const destPath = `$DEST/${safeName}`;
			// Convert the source path, replacing the drive letter with wslSource
			...
			if (entry.fileMask) {
				// Handle wildcards - use find + cp for complex patterns
				const pattern = entry.fileMask.replace("*", "");
				if (entry.fileMask.startsWith("*") && entry.fileMask.endsWith("*")) {
					lines.push(
						`find "${sourcePath}" -type f -name "${entry.fileMask}" -exec cp {} "${destPath}/" \\; 2>/dev/null`,
					);
				} else if (entry.fileMask.includes("*")) {
					lines.push(
		...
		<div className="border border-border bg-card/20 rounded-lg">
			...
			<div className="p-4">
				<div className="relative">
					<pre className="p-4 bg-black/50 text-xs overflow-x-auto max-h-96 scrollbar-thin">
						<code className="text-green-400 whitespace-pre-wrap">
							{(format === "kape" || format === "wsl") && (
					...
					<button
						...
						className="absolute top-2 right-2 px-2 py-1 text-[10px] text-muted-foreground hover:text-foreground bg-secondary/50 border border-border transition-colors focus-ring"
						aria-label={
